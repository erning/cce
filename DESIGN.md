...
7. **Source.** The env file is sourced into the current shell. An `ERR`
   ...
   prints a context line naming the file; `set -e` then aborts cce.
   The trap is cleared immediately after the source returns.
...
9. **Exec.** `exec "$_CCE_COMMAND" ${_CCE_ARGS[@]+"${_CCE_ARGS[@]}"}`
...
The run path is short and unadorned:
...
### Why cce stays simple

cce is a personal wrapper whose only job is to splice a user-owned
`.env` file in front of a target command. The env file is bash that
the user wrote (or explicitly pasted in from somewhere they trust);
it is **not** an untrusted input, and cce does not try to sandbox it.
If you source it, you trust it — same contract as `~/.bashrc`,
`~/.envrc`, or any script you `.` into your current shell.

Earlier iterations of this file chased a different model. After
reviews flagged that "an env file could define `exec() { … }` and
hijack the final `exec`", or "an env file could do `set +e; false`
and sneak past the ERR trap", the script grew `builtin` prefixes on
every post-source control-flow call, an ERR trap body that actively
exited, a post-source `set -euo pipefail` restore, and a smoke
battery full of adversarial fixtures. That work was all reverted in
v2.1.6. Two reasons:

1. **It was defending the user against their own config.** If you
   write `exec() { … }` in a file you explicitly pasted into
   `~/.config/cce/`, that is not sabotage you need a tool to guard
   against. It is you changing the shell, on purpose, in your own
   file.
2. **Bash is too flexible to meaningfully sandbox without a
   subshell.** Each patch opened a new edge case (`builtin` can be
   shadowed too, so defend `builtin` — but then `builtin builtin`
   can also be shadowed, etc.), and the code was growing faster
   than the threats were shrinking.

When a future review flags a finding of the form *"but an env file
could still do X"*, the right response is almost always to add a
line here and move on. The correctness bar for cce is **ordinary
misuse**: missing file, syntax error, a plain `false` inside an env
file, target command not on `PATH`. Those paths all produce a clear
error. Weird bash tricks inside the env file are the user's problem.
...
`set -e` active inside the sourced file, so partial loads abort with
a clear context line.

One failure mode the runtime `ERR` trap does not catch: a Bash *parse*
error inside the env file, because no command actually runs. We catch
those upfront with a `bash -n "$_CCE_ENV_FILE"` syntax pre-check that
prints the bad-syntax error indented under a clear `cce` context line.
...
immediately before the env file is sourced.

The prefix is pure collision avoidance: a user may legitimately want
to set `ENV_NAME=production` or `COMMAND=my-tool` in their env file
for their own reasons, and both are common names. The `_CCE_` prefix
keeps cce's own state out of that namespace.

`readonly` on top turns an accidental `_CCE_COMMAND=...` inside an
env file into an early bash error (`readonly variable`) instead of a
silent override of cce's state. This is cheap, it catches a specific
class of typo, and it is not meant as a defence against deliberately
hostile env files.
...
- **Sandboxing env files, or defending against env-file "attacks".**
  An env file is ordinary bash, sourced into cce's own shell with
  full language access: function definitions, `trap`, `set`, command
  substitution, everything. cce does **not** try to guard against an
  env file that shadows `exec`, disarms the `ERR` trap, toggles
  `set -e`, hits an expansion error, or otherwise messes with cce's
  control flow. If you wrote the file, you trust it — same contract
  as `~/.bashrc` or a `.envrc` snippet. See "Why cce stays simple"
  under `## Source and exec` above for the history of why this is
  spelled out so forcefully: each patch in that direction opened a
  new edge case and grew the script without actually raising the
  security bar.
